...
from dataset.dataset_3d_gen import Dataset3dGen
...
def optimize_mesh(
    ...
    for epoch in range(num_epoches):
        for it, target in enumerate(dataloader_train):
            ...
            #  Training
            # ==============================================================================================
            total_it = it + epoch * len(dataloader_train)
            img_loss, reg_loss, log = trainer(target, total_it)
            ...
            smooth_eng_vec.append(log['smooth_eng'])
            barrier_eng_vec.append(log['barrier'])
            kd_smooth_vec.append(log['kd_smooth'])
            visibility_reg_vec.append(log['visibility_reg'])
            light_reg_vec.append(log['light_reg'])
            ...
            if it % log_interval == 0 and FLAGS.local_rank == 0:
                ...
                smooth_eng_avg = np.mean(np.asarray(smooth_eng_vec[-log_interval:]))
                barrier_eng_avg = np.mean(np.asarray(barrier_eng_vec[-log_interval:]))
                kd_smooth_avg = np.mean(np.asarray(kd_smooth_vec[-log_interval:]))
                visibility_reg_avg = np.mean(np.asarray(visibility_reg_vec[-log_interval:]))
                light_reg_avg = np.mean(np.asarray(light_reg_vec[-log_interval:]))
                iter_dur_avg = np.mean(np.asarray(iter_dur_vec[-log_interval:]))
                
                remaining_time = (FLAGS.iter-it)*iter_dur_avg
                print("epoch=%3d, iter=%5d, \
                       img_loss=%.6f, \
                       reg_loss=%.6f, \
                       smooth_eng=%.6f, \
                       barrier_eng=%.6f, \
...
if __name__ == "__main__":
    ...
    data_path = "/research/cbim/vast/zz500/Projects/mhg/skel-shape-gen/AllData"
    expr_prompt = "a_white_dog"
    file_path = "{}/gen_mv_imgs_nerf_{}".format(data_path, expr_prompt)
    dataset_train = Dataset3dGen(file_path, FLAGS, examples=None)
    dataset_validate = Dataset3dGen(file_path, FLAGS, examples=None)
    ...
    if FLAGS.base_mesh is None:
        ...
        geometry, mat = optimize_mesh(glctx, geometry, mat, lgt, dataset_train, dataset_validate, 
                        FLAGS, pass_idx=0, pass_name="tet_pass1", optimize_light=FLAGS.learn_light)

        if FLAGS.local_rank == 0 and FLAGS.validate:
            validate(glctx, geometry, mat, lgt, dataset_validate, os.path.join(FLAGS.out_dir, "tet_validate"), FLAGS)
        ...
        if FLAGS.local_rank == 0:
            ...
            os.makedirs(os.path.join(FLAGS.out_dir, "tet_mesh"), exist_ok=True)
            obj.write_obj(os.path.join(FLAGS.out_dir, "tet_mesh/"), base_mesh)
            light.save_env_map(os.path.join(FLAGS.out_dir, "tet_mesh/probe.hdr"), lgt)
